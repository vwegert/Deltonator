/**********************************************************************************************************************
 **
 ** parts/printed/duet_holder.scad
 **
 ** This file constructs the holder for the DuetWifi board
 **
 **********************************************************************************************************************/
 ...
 */
module _duet_holder_frame_nut_hole(head_rotation = 0) {
 ...
 */
module _duet_holder_frame_hstrut() {
 ...
 */
module _duet_holder_frame_vstrut() {
	difference() {
		// the strut
		...
		translate([duet_holder_strut_width() / 2,
			       ...
			       0]) 
			_duet_holder_frame_nut_hole(head_rotation = 90);
		translate([duet_holder_strut_width() / 2,
 ...
 */
module _duet_holder_wall_right() {
	translate([duet_pcb_mount_width() - duet_holder_wall_thickness(),
		...
		cube([duet_holder_wall_thickness(),
			  duet_holder_size_y(),
			  duet_holder_size_z_inner()]);
		// the outer wall is lowered here to allow the components to protrude!
		translate([duet_holder_wall_thickness(),
				   0,
				   0])
			cube([duet_holder_wall_thickness(),
 ...
 */
module _duet_holder_wall_bottom() {
	difference() {
		// the wall...
		union() {
	...
	color_printed_psd_case()
		render() {
			union() {
				...
				// the walls
				_duet_holder_wall_left();
				_duet_holder_wall_right();
				_duet_holder_wall_bottom();
				_duet_holder_wall_top();
 ...
 */
module duet_holder() {
	bom_entry(section = "Printed Parts", description = "Electronics Mount", size = "Duet Wifi Holder");
	color_printed_psd_case()
		import(file = "duet_holder.stl");
}

/**
 * Renders a nut to be placed in the base struts.
 */
module _duet_holder_nut_screw(head_rotation = 0, screw_material_thickness = 6) {
 ...
 */
module duet_holder_hardware(screw_material_thickness = 6) {
	...
	translate([duet_holder_strut_offset() + duet_holder_strut_size_x() * 0.25, 
			   ...
			   0])
		_duet_holder_nut_screw(screw_material_thickness = screw_material_thickness);
	...
	translate([duet_holder_strut_offset() + duet_holder_strut_width() / 2, 
			   ...
			   0])
		_duet_holder_nut_screw(head_rotation = 90, screw_material_thickness = screw_material_thickness);
	translate([duet_holder_strut_offset() + duet_holder_strut_width() / 2, 
 ...
 */
module duet_holder_punch(hole_depth = 100) {
